...
export default function NewProductPage() {
    ...
    const [formData, setFormData] = useState({
        ...
        description_fr: '',
        description_ar: '',
        price: '', // This will be the selling price (Promo Price if is_promo is true)
        original_price: '', // This will be the crossed out price
        is_promo: false,
        category: '',
        subcategory: '',
        stock: '',
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Logic for prices
            // If is_promo:
            // - price = "Prix Promo" field
            // - original_price = "Prix Normal" field
            // If NOT is_promo:
            // - price = "Prix Normal" field
            // - original_price = null (or ignored by backend if we send it)

            // Wait, simplification: 
            // - formData.price is ALWAYS the selling price.
            // - formData.original_price is the Crossed Out price (only valid if Promo).

            // Let's assume the user fills:
            // "Prix" -> 20 (Original)
            // Checkbox "Promo" -> Checked
            // "Prix Promo" -> 15 (New Selling Price)

            // In DB: price=15, original_price=20.

            // BUT, the fields in form should be intuitive.
            // Standard Field: "Prix" (The main price).
            // If Promo Checked: "Prix Promo" appears.

            // Let's refine the handleSubmit to map this correctly.
            // I will use `formData.price` as the "Prix" input (Standard)
            // and `formData.promo_price_input` (new temp state) as the promo price.
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="relative w-40 h-40 border-2 border-dashed border-zinc-700 rounded-xl flex flex-col items-center justify-center cursor-pointer hover:border-primary transition-colors overflow-hidden group">
                            ...
                            {imagePreview ? (
                                ...
                                <>
                                    ...
                                    <span className="text-xs text-zinc-500">Ajouter une image</span>
                                </>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="bg-zinc-900/50 p-6 rounded-xl border border-zinc-800">
                        <div className="flex items-center gap-3 mb-6">
                            ...
                            />
                            <label htmlFor="is_promo" className="font-bold text-white select-none cursor-pointer">
